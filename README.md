...
- **Hotkeys:** `1`–`9` (action selection), `Q` (quest journal), `P` (party & companions), `H` (bestiary & apex hunts), `K` (survival camp & foraging), `O` (faction orders & war banners), `G` (ancient shrines & divine blessings), `L` (survey landmarks & panoramas), `X` (ancient codex), `T` (commodity exchange), `W` (weather forecast), `B` (mercenary contracts), `M` (continental map), `C` (character sheet), `U` (undo), `Escape` (close modals).
