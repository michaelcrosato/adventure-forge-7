...
- **Hotkeys:** `1`–`9` (action selection), `Q` (quest journal), `P` (party & companions), `H` (bestiary & apex hunts), `K` (survival camp & foraging), `O` (faction orders & war banners), `G` (ancient shrines & divine blessings), `L` (survey landmarks & panoramas), `V` (dungeon vaults & crypt keystones), `F` (runeforges & crucible anvils), `E` (alchemical laboratories & elixirs), `X` (ancient codex), `T` (commodity exchange), `W` (weather forecast), `B` (mercenary contracts), `M` (continental map), `C` (character sheet), `U` (undo), `Escape` (close modals).
